...
import { RegionComponentsPage, RegionDeleteDialog, RegionUpdatePage } from './region-deer-inc.page-object';
...
describe('Region e2e test', () => {
  ...
  it('should load Regions', async () => {
    await navBarPage.goToEntity('region-deer-inc');
    regionComponentsPage = new RegionComponentsPage();
    await browser.wait(ec.visibilityOf(regionComponentsPage.title), 5000);
    expect(await regionComponentsPage.getTitle()).to.eq('deerDashApp.region.home.title');
  ...
  it('should load create Region page', async () => {
    ...
    expect(await regionUpdatePage.getPageTitle()).to.eq('deerDashApp.region.home.createOrEditLabel');
  ...
  it('should delete last Region', async () => {
    ...
    expect(await regionDeleteDialog.getDialogTitle()).to.eq('deerDashApp.region.delete.question');
