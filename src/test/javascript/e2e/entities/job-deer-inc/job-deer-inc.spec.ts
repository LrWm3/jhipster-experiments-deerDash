...
import { JobComponentsPage, JobDeleteDialog, JobUpdatePage } from './job-deer-inc.page-object';
...
describe('Job e2e test', () => {
  ...
  it('should load Jobs', async () => {
    await navBarPage.goToEntity('job-deer-inc');
    jobComponentsPage = new JobComponentsPage();
    await browser.wait(ec.visibilityOf(jobComponentsPage.title), 5000);
    expect(await jobComponentsPage.getTitle()).to.eq('deerDashApp.job.home.title');
  ...
  it('should load create Job page', async () => {
    ...
    expect(await jobUpdatePage.getPageTitle()).to.eq('deerDashApp.job.home.createOrEditLabel');
  ...
  it('should delete last Job', async () => {
    ...
    expect(await jobDeleteDialog.getDialogTitle()).to.eq('deerDashApp.job.delete.question');
