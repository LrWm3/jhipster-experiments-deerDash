...
import { TaskComponentsPage, TaskDeleteDialog, TaskUpdatePage } from './task-deer-inc.page-object';
...
describe('Task e2e test', () => {
  ...
  it('should load Tasks', async () => {
    await navBarPage.goToEntity('task-deer-inc');
    taskComponentsPage = new TaskComponentsPage();
    await browser.wait(ec.visibilityOf(taskComponentsPage.title), 5000);
    expect(await taskComponentsPage.getTitle()).to.eq('deerDashApp.task.home.title');
  ...
  it('should load create Task page', async () => {
    ...
    expect(await taskUpdatePage.getPageTitle()).to.eq('deerDashApp.task.home.createOrEditLabel');
  ...
  it('should delete last Task', async () => {
    ...
    expect(await taskDeleteDialog.getDialogTitle()).to.eq('deerDashApp.task.delete.question');
