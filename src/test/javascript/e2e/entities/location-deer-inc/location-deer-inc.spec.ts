...
import { LocationComponentsPage, LocationDeleteDialog, LocationUpdatePage } from './location-deer-inc.page-object';
...
describe('Location e2e test', () => {
  ...
  it('should load Locations', async () => {
    await navBarPage.goToEntity('location-deer-inc');
    locationComponentsPage = new LocationComponentsPage();
    await browser.wait(ec.visibilityOf(locationComponentsPage.title), 5000);
    expect(await locationComponentsPage.getTitle()).to.eq('deerDashApp.location.home.title');
  ...
  it('should load create Location page', async () => {
    ...
    expect(await locationUpdatePage.getPageTitle()).to.eq('deerDashApp.location.home.createOrEditLabel');
  ...
  it('should delete last Location', async () => {
    ...
    expect(await locationDeleteDialog.getDialogTitle()).to.eq('deerDashApp.location.delete.question');
