...
import com.deerinc.cloud.config.KafkaProperties;
...
class DeerDashKafkaResourceIT {
    ...
    void setup() {
        ...
        DeerDashKafkaResource kafkaResource = new DeerDashKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/deer-dash-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/deer-dash-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
